...
module.exports = class QueryStream extends Readable {
  ...
  constructor (client, sql, opts) {
    ...
    this._sql = sql
    this._useDeserialize = !opts.disableDeserialize
  ...
  _land (err, result) {
    ...
    if (!result.nextUri) {
      ...
      try {
        nextUri = new URL(result.nextUri)
      } catch (err) {
        // Skip cancelation
        this._errored = true
        return this.destroy(new Error('Presto sent invalid nextUri: ' + result.nextUri))
      }
      ...
      this._requestOptions = {
        ...
        port: nextUri.port ? parseInt(nextUri.port, 10) : undefined,
        path: path,
        method: 'GET',
        json: true,
        expectStatusCode: 200
