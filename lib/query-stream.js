...
const RETRY_ERROR_CODES = new Set([
  ...
  // Occasionally seen in production, not caused by human cancelation. Could
  // have been a lento bug (since fixed) but has also been reported by users
  // of other Presto clients.
  'USER_CANCELED'
])
...
module.exports = class QueryStream extends Readable {
  ...
  _land (err, result) {
    ...
    if (err) {
      ...
      if (!this._received && RETRY_ERROR_CODES.has(err.code) && this._backoff.attempts < this._client.maxRetries) {
        this._reset(true)
        this._retryTimeout = setTimeout(this._takeoff, this._backoff.duration())
      } else {
        // TODO: skip destroy() if called?
        this.emit('error', err)
      }
    ...
    if (!result.nextUri) {
      ...
      const nextUri = url.parse(result.nextUri)

      this._prevPath = this._requestOptions.path

      this._requestOptions.path = nextUri.path
      this._requestOptions.method = 'GET'
      this._requestOptions.body = undefined

      // Follow host redirects, but don't switch protocols.
      this._requestOptions.hostname = nextUri.hostname
      this._requestOptions.port = nextUri.port ? parseInt(nextUri.port, 10) : null
    }
  ...
  _cancel (queryId, err, callback) {
    ...
    const finish = (err) => {
      ...
      // TODO: update to readable-stream@3 (mirroring node v10) and
    ...
    if (!queryId) return process.nextTick(finish, err)
    ...
    this._client.request(request, (cancelationErr) => {
      if (err && cancelationErr) finish(new AggregateError([err, cancelationErr]))
      else finish(err || cancelationErr)
    })
  }
}
