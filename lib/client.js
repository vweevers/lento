  ...
  _makeRequest (opts, backoff, callback_) {
    ...
    const req = (opts.protocol === 'http:' ? http : https).request(opts, (res) => {
      ...
      if (statusCode === 307) {
        ...
        if (!location) {
          return callback(new Error('http 307 redirect is missing "location" header'))
        }

        try {
          redirectUrl = new URL(location)
        } catch (err) {
          return callback(new Error('http 307 redirect has invalid "location" header: ' + location))
        }

        if (opts.protocol !== redirectUrl.protocol) {
          return callback(new Error('http 307 redirect protocol switch is not allowed'))
        }
      ...
      if (statusCode === 503) {
        return retry(herr(statusCode, { expose: false }), 50, 100)
      }
