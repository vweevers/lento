...
const flattenPage = require('./flatten-page')
...
module.exports = class Client {
  ...
  request (opts, callback) {
    ...
    if (opts.method === 'POST' && this._session.size > 0) {
      ...
      // TODO: do not reuse `opts` and `headers` objects
    ...
    this._makeRequest(opts, null, function callbackOnce (err, result) {
      if (callback !== null) {
        callback(err, result)
        callback = null
      }
  ...
  _makeRequest (opts, backoff, callback) {
    const req = (opts.protocol === 'http:' ? http : https).request(opts, (res) => {
      ...
      if (statusCode === 503) {
        const bo = backoff || new Backoff({ min: 50, max: 100 })
        const fn = this._makeRequest.bind(this, opts, bo, callback)

        if (bo.attempts >= this.maxRetries) {
          // TODO: use `http-errors` to attach status code to error
          return callback(new Error(`http 503 (retried ${bo.attempts} times)`))
        }

        return setTimeout(fn, bo.duration())
      }
